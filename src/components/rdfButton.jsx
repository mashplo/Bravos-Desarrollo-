      ...
      >
        <div className="modal-box max-w-3xl">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <FileText /> RDF del sitio
          </h3>
          <div className="flex gap-2 mt-3">
            <button
              className={`btn btn-sm ${
                format === "turtle" ? "btn-primary" : "btn-ghost"
              }`}
              onClick={() => fetchRdf("turtle")}
            >
              Turtle (.ttl)
            </button>
            <button
              className={`btn btn-sm ${
                format === "xml" ? "btn-primary" : "btn-ghost"
              }`}
              onClick={() => fetchRdf("xml")}
            >
              RDF/XML (.rdf)
            </button>
            ...
            <a
              ...
              href={`${backend}/api/rdf?format=${format}`}
              target="_blank"
              rel="noreferrer"
            >
            ...
            <button
              className="btn btn-sm btn-ghost"
              onClick={() => window.open(`${backend}/api/grafo`, "_blank")}
            >
              <ExternalLink size={14} className="mr-2" />
              Ver Grafo
            </button>
          ...
          <div className="mt-4 flex gap-4">
            <div className="w-1/3 hidden md:block">
              ...
              <img
                src={`${backend}/api/grafo`}
                alt="Grafo"
                className="w-full h-48 object-contain rounded shadow"
              />
            </div>
            <div className="flex-1">
              <pre className="whitespace-pre-wrap max-h-96 overflow-auto bg-base-200 p-3 rounded text-sm">
                {content}
              </pre>
            </div>
          </div>
