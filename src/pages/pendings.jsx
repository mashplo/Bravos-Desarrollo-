...
export default function Pendings() {
  ...
  const cargar_pedidos = async () => {
    ...
    const pendientes = todos_pedidos.filter(
      (p) => p.estado === "en_preparacion" || p.estado === "enviado"
    );
  ...
  const reiniciar_contador = async () => {
    if (
      !confirm(
        "⚠️ ADVERTENCIA: Esto borrará TODOS los pedidos y reiniciará el contador a 1. ¿Estás seguro?"
      )
    ) {
      return;
    }
              ...
              <ul className="flex flex-col">
                {pedidos_pendientes.length === 0 ? (
                  ...
                  pedidos_pendientes.map((pedido) => (
                    <li
                      key={pedido.id}
                      className="flex flex-col gap-3 my-5 p-4 border rounded-lg bg-base-100 shadow"
                    >
            ...
            <ul className="flex flex-col gap-6">
              {pedidos_entregados.length === 0 ? (
                ...
                pedidos_entregados.map((pedido) => (
                  <li
                    key={pedido.id}
                    className="flex flex-col gap-3 p-4 bg-base-100 text-base-content rounded-lg shadow"
                  >
                    <div className="flex justify-between items-center mb-2">
                      <div className="flex flex-col">
                        <span className="text-sm text-gray-500">
                        ...
                        <span className="text-base font-bold text-success mt-1">
                          💳 {pedido.metodo_pago}
                        </span>
                      </div>
